<template>
  ...
  >
    <UCard
      class="transition-all duration-300 hover:shadow-xl h-full border-0 overflow-hidden"
      :ui="{
        body: { padding: 'p-0' },
      ...
      <div class="flex flex-col h-full">
        <div class="aspect-video bg-gray-100 relative overflow-hidden">
          ...
          <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 bg-black/40 transition-opacity duration-300">
            <div class="w-16 h-16 bg-lime-500 rounded-full flex items-center justify-center transform group-hover:scale-110 transition-transform duration-300">
              <UIcon
                name="i-lucide-play"
                size="lg"
                class="text-white"
        ...
        <div class="p-5 flex-1 flex flex-col">
          <div class="flex items-start justify-between mb-2 gap-2">
            <h3 class="text-lg font-bold line-clamp-2 group-hover:text-lime-500 transition-colors duration-300">
              {{ video.title }}
            </h3>
          </div>

          <p class="text-gray-600 text-sm line-clamp-2 mb-3">{{ video.description }}</p>
